...
def _precios_sucursal_y_publico(precio_unitario, total_partida=None, cantidad=None, precio_publico_lista=None):
    ...
    """
    precio_suc = _to_float(precio_unitario)
    total_suc = _to_float(total_partida)
    ...
    precio_pub = (
        round(precio_suc * PRECIO_PUBLICO_FACTOR, 2)
        if precio_suc is not None else None
    )
    total_pub = (
        round(total_suc * PRECIO_PUBLICO_FACTOR, 2)
    ...
    diferencia = (
        round(total_pub - total_suc, 2)
        if total_pub is not None and total_suc is not None else None
    )
    return precio_suc, total_suc, precio_pub, total_pub, diferencia
    ...
    *,
    ...
    for p in pedidos:
        ...
        for d in detalles_map.get(p.id, []):
            ...
            pedido_rows.append({
                ...
                'total_partida': partida_total,
                'precio_sucursal_unitario': precio_suc,
                'total_partida_precio_sucursal': total_suc if total_suc is not None else partida_total,
                'precio_publico_unitario': precio_pub,
                'total_partida_precio_publico': total_pub,
                'diferencia_precio_publico': diferencia,
                'serie': '',
                'folio': p.name,
    ...
    if page == 1:
        ...
        for f in faltantes_indice:
            ...
            precio_lista = public_price_map.get(_norm_txt(f.get('clave_producto')))
            precio_suc, total_suc, precio_pub, total_pub, diferencia = _precios_sucursal_y_publico(
                precio,
            ...
            target_item['detalles'].append({
                ...
                'total_partida': total_partida,
                'precio_sucursal_unitario': precio_suc,
                'total_partida_precio_sucursal': total_suc if total_suc is not None else total_partida,
                'precio_publico_unitario': precio_pub,
                'total_partida_precio_publico': total_pub,
                'diferencia_precio_publico': diferencia,
            ...
            target_item['pedido_total_precio_publico'] = round(
                target_item['pedido_total'] * PRECIO_PUBLICO_FACTOR, 2
            )
...
def _compute_conciliacion_resumen_from_items(items):
    ...
    for it in items:
        ...
        # Preferir totales a nivel pedido para que el panel superior coincida
        # con el pie de cada tarjeta.
        ...
        if total_pub is None and total_suc is not None:
            total_pub = round(total_suc * PRECIO_PUBLICO_FACTOR, 2)
    ...
    try:
        ...
        for p in pedidos:
            ...
            for d in detalles_map.get(p.document_id, []):
                ...
                precio_unit = _to_float(d.precio_unitario)
                precio_lista = public_price_map.get(_norm_txt(d.clave_producto))
                precio_suc, total_suc, precio_pub, total_pub, diferencia = _precios_sucursal_y_publico(
                    precio_unit,
                    total_partida=partida_total,
                    cantidad=cantidad_num,
            ...
            items.append({
                ...
                'pedido_total': round(pedido_total, 2),
                'pedido_total_precio_sucursal': round(pedido_total, 2),
                'pedido_total_precio_publico': round(pedido_total * PRECIO_PUBLICO_FACTOR, 2),
                'detalles': pedido_rows,
